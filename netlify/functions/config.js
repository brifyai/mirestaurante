...
function formatConfigForFrontend(config) {
  ...
  console.log(
    "🔍 formatConfigForFrontend - groq_api_key:",
    config?.groq_api_key,
  );
  ...
  console.log(
    "🔍 formatConfigForFrontend - result.apis.groq:",
    result.apis.groq,
  );
  console.log(
    "🔍 formatConfigForFrontend - result.aiSettings.model:",
    result.aiSettings.model,
  );
  ...
  try {
    ...
    if (httpMethod === "POST") {
      ...
      const configData = {
        ...
        business_website:
          body.businessInfo?.website || "https://jaraquemada.com",
